...
여러 `catch`문 중에서 발생한 예외 타입과 일치하는 첫 번째 `catch`문만 실행된다.  
`finally`문은 예외 발생 여부와 관계없이 항상 실행된다.
...
자식 생성자의 첫 줄에는 자동으로 `super()`가 호출된다.  
(단, 명시적으로 `super(...)` 또는 `this(...)`를 작성한 경우 제외)

따라서 부모 클래스의 초기화와 부모 생성자가 먼저 실행된다.
...
> 🔎 **짚고 넘어가야 할 부분**  
> `child` 필드 초기화는 `parent()` 생성자 실행이 끝난 후에 수행된다.  
{: .prompt-tip }
...
**오버라이딩** (Overriding)  
상위 클래스의 메서드를 하위 클래스에서 재정의하여,  
동일한 메서드 호출 시 다른 동작을 수행하도록 하는 것 (동적 바인딩)

<br>

### static 메서드는 오버라이딩되지 않으며, hiding이 발생한다.

부모 클래스의 `static` 메서드는 오버라이딩이 되지 않고,  
자식 클래스에 동일한 이름의 메서드를 정의하면  
부모의 메서드를 숨기는(`hiding`) 효과가 발생한다.

`static` 메서드는 객체가 아닌 클래스에 바인딩되기 때문에,  
참조 변수의 실제 객체가 아닌 선언된 타입 기준으로 결정된다.

> `static` 메서드는 컴파일 시점에 바인딩된다. (정적 바인딩)

<br>

### implements, extends는 언제 사용할까?

- `class`는 `interface`를 구현하며,  
  인터페이스에 정의된 메서드를 반드시 구현해야 한다.
...
- `class`는 `class`를 상속하며,  
  부모 클래스의 필드와 메서드를 물려받아 사용할 수 있다.
...
- `interface`는 `interface`를 확장하며,  
  기존 인터페이스의 메서드를 상속받아 새로운 인터페이스를 정의할 수 있다
...
> `class`는 여러 `interface`를 구현할 수 있지만, `class`의 다중 상속은 불가능하다.  
{: .prompt-tip }
